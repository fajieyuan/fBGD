...
import data.gla.uni.data.structure.complementary.Pair;
 ...
 * @version 1.1
 */
public class SSparseMatrix implements Serializable{
	...
	private SSparseVector[] rows;
	/** The array of column references. */
	private SSparseVector[] cols;
	 ...
	 */
	public SSparseMatrix(int m, int n) {
		this.M = m;
		this.N = n;
		rows = new SSparseVector[M];
		cols = new SSparseVector[N];
		
		for (int i = 0; i < M; i++) {
			rows[i] = new SSparseVector(N);
		}
		for (int j = 0; j < N; j++) {
			cols[j] = new SSparseVector(M);
		}
	}
	 ...
	 */
	public SSparseMatrix(SSparseMatrix sm) {
		this.M = sm.M;
		this.N = sm.N;
		rows = new SSparseVector[M];
		cols = new SSparseVector[N];
	 ...
	 * @param newVector 
	 */
	public void setRowVector(int i, SSparseVector newVector) {
	 ...
	 * @param newVector 
	 */
	public void setRowVectorNonnegative(int i, SSparseVector newVector) {
		if (newVector.length() != this.N)
			throw new RuntimeException("Vector lengths disagree");
		if (i < 0 || i >= this.M)
			throw new RuntimeException("Wrong input row index.");
	 ...
	 */
	public void setColVector(int j, SSparseVector newVector) {
	 ...
	 */
	public SSparseVector getRowRef(int index) {
		return rows[index];
	}
	 ...
	 * @return A reference to the designated row.
	 */
	public SSparseVector getRow(int index) {
		SSparseVector newVector = this.rows[index].copy();
	 ...
	 */
	public SSparseVector getColRef(int index) {
		return cols[index];
	}
	 ...
	 * @return A reference to the designated column.
	 */
	public SSparseVector getCol(int index) {
		SSparseVector newVector = this.cols[index].copy();
	 ...
	 **/
	public SSparseVector getRowAverage(double defalut_value) {
		SSparseVector rowAverage = new SSparseVector(this.M);
		for (int u = 0; u < this.M; u++) {
			SSparseVector v = this.getRowRef(u);
	 ...
	 */
	public SSparseVector getColumnAverage(double defalut_value) {
		SSparseVector columnAverage = new SSparseVector(this.N);
		for (int i = 0; i < this.N; i++) {
			SSparseVector j = this.getColRef(i);
	 ...
	 */
	public SSparseVector diagonal() {
		SSparseVector v = new SSparseVector(Math.min(this.M, this.N));
	...
	public double max() {
		...
		for (int i = 0; i < this.M; i++) {
			SSparseVector v = this.getRowRef(i);
			if (v.itemCount() > 0) {
				double rowMax = v.max();
				if (v.max() > curr) {
					curr = rowMax;
	 ...
	 */
	public SSparseMatrix scale(double alpha) {
		SSparseMatrix A = new SSparseMatrix(this.M, this.N);
	 ...
	 * @param alpha The scalar value to be multiplied to this matrix.
	 */
	public SSparseMatrix selfScale(double alpha) {
		for (int i = 0; i < this.M; i++) {
			ArrayList<Integer> itemList = this.getRowRef(i).indexList();
			for (int j : itemList) {
	 ...
	 */
	public SSparseMatrix add(double alpha) {
		SSparseMatrix A = new SSparseMatrix(this.M, this.N);
	 ...
	 */
	public SSparseMatrix exp(double alpha) {
		for (int i = 0; i < this.M; i++) {
			SSparseVector b = this.getRowRef(i);
	 ...
	 */
	public SSparseMatrix transpose() {
		SSparseMatrix A = new SSparseMatrix(this.N, this.M);
	 ...
	 */
	public SSparseVector times(SSparseVector x) {
		if (N != x.length())
			throw new RuntimeException("Dimensions disagree");
		
		SSparseMatrix A = this;
		SSparseVector b = new SSparseVector(M);
	 ...
	 */
	public SSparseMatrix times(SSparseMatrix B) {
		// original implementation
		if (N != (B.length())[0])
			throw new RuntimeException("Dimensions disagree");
		
		SSparseMatrix A = this;
		SSparseMatrix C = new SSparseMatrix(M, (B.length())[1]);
		for (int i = 0; i < M; i++) {
			for (int j = 0; j < (B.length())[1]; j++) {
				SSparseVector x = A.getRowRef(i);
				SSparseVector y = B.getColRef(j);
	 ...
	 * @return
	 */
	public SSparseMatrix dotTimes(SSparseMatrix B) {
		if (M != B.M || N != B.N) {
			throw new RuntimeException("dotTimes: Matrices are not of the same size!");
		}
		SSparseMatrix C = new SSparseMatrix(M, N);
		for (int i = 0; i < M; i ++) {
	 ...
	 * @return
	 */
	public SSparseMatrix dotDivide(SSparseMatrix B) {
		if (M != B.M || N != B.N) {
	 ...
	 */
	public SSparseMatrix rowStochastic() {
		SSparseMatrix C = new SSparseMatrix(M, N); 
		for (int i = 0; i < M; i++) {
	 ...
	 * @return
	 */
	public SSparseMatrix rowL2Norm() {
		SSparseMatrix C = new SSparseMatrix(M, N);
		for (int i = 0; i < M; i++) {
	 ...
	 */
	public SSparseMatrix colStochastic() {
		SSparseMatrix C = new SSparseMatrix(this.M, this.N);
	 ...
	 * @throws RuntimeException when dimensions disagree
	 */
	public void selfTimes(SSparseMatrix B) {
		// original implementation
		if (N != (B.length())[0])
			throw new RuntimeException("Dimensions disagree");
		
		for (int i = 0; i < M; i++) {
			SSparseVector tmp = new SSparseVector(N);
			for (int j = 0; j < (B.length())[1]; j++) {
				SSparseVector x = this.getRowRef(i);
				SSparseVector y = B.getColRef(j);
	 ...
	 */
	public SSparseMatrix plus(SSparseMatrix B) {
		SSparseMatrix A = this;
		if (A.M != B.M || A.N != B.N)
			throw new RuntimeException("Dimensions disagree");
	 ...
	 */
	public SSparseMatrix minus(SSparseMatrix B) {
		SSparseMatrix A = this;
		if (A.M != B.M || A.N != B.N)
			throw new RuntimeException("Dimensions disagree");
	 ...
	 */
	public static SSparseMatrix makeIdentity(int n) {
		SSparseMatrix m = new SSparseMatrix(n, n);
		for (int i = 0; i < n; i++) {
			m.setValue(i, i, 1.0);
		}
	 ...
	 * @return
	 */
	public static SSparseMatrix makeUniform(int M, int N) {
		SSparseMatrix m = new SSparseMatrix(M, N);
		for (int i = 0; i < M; i ++) {
			for (int j = 0; j < N; j++) {
	 ...
	 * @return
	 */
	public static SSparseMatrix makeRandom(int M, int N, double sparseRate) {
	 ...
	 */
	public SSparseMatrix inverse() {
		if (this.M != this.N)
			throw new RuntimeException("Dimensions disagree");
		
		SSparseMatrix original = this;
		SSparseMatrix newMatrix = makeIdentity(this.M);
		...
		SSparseMatrix b = new SSparseMatrix(original);
	...
	public SSparseMatrix cholesky() {
		...
		int n = A.M;
		SSparseMatrix L = new SSparseMatrix(n, n);
	 ...
	 */
	public SSparseMatrix covariance() {
		int columnSize = this.N;
		SSparseMatrix cov = new SSparseMatrix(columnSize, columnSize);
		
		for (int i = 0; i < columnSize; i++) {
			for (int j = i; j < columnSize; j++) {
				SSparseVector data1 = this.getCol(i);
				SSparseVector data2 = this.getCol(j);
	 ...
	 * @return The resulting matrix after scaling.
	 */
	public SSparseMatrix partScale(double alpha, int[] indexList) {
		if (indexList != null) {
			for (int i : indexList) {
				for (int j : indexList) {
					this.setValue(i, j, this.getValue(i, j) * alpha);
	 ...
	 * @return The resulting matrix after summation.
	 */
	public SSparseMatrix partPlus(SSparseMatrix B, int[] indexList) {
		if (indexList != null) {
	 ...
	 */
	public SSparseMatrix partMinus(SSparseMatrix B, int[] indexList) {
		if (indexList != null) {
			if (this.M != B.M || this.N != B.N)
				throw new RuntimeException("Dimensions disagree");
	 ...
	 * @return The resulting vector after multiplication.
	 */
	public SSparseVector partTimes(SSparseVector x, int[] indexList) {
		if (indexList == null)
			return x;
	...
	public String toString() {
        ...
        for (int i = 0; i < this.M; i++) {
        	SSparseVector row = this.getRowRef(i);
