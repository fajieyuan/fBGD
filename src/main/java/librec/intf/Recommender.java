...
public abstract class Recommender implements Runnable {
	...
	protected int user_size, item_size,tag_size, x_size, k0, k1, k2, x3size, x4size,
	x5size, PC, PI;
    protected double loss_threshold;//fajie
    protected static int numIters;//fajie
    protected SSparseMatrix ZX;//fajie
    protected SSparseMatrix ZY;//fajie
	...
	public Recommender(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
		...
		if (isRankingPred && isDiverseUsed)
         ...
         if(algoName.toLowerCase().equals("icd_fm")||
        		algoName.toLowerCase().equals("rankfm_nextmusic")||
        		algoName.toLowerCase().equals("adafm_nextmusic")	){
        	PC = algoOptions.getInt("-pc");
    		PI = algoOptions.getInt("-pi");
    		k0 = algoOptions.getInt("-k0");
			k1 = algoOptions.getInt("-k1");
			k2 = algoOptions.getInt("-k2");
        	x_size=PC+PI+2;//usually a big V is no problem, we here add 2 consider
        }
	}
	...
	/**
	 * fajie add 
	 * To calculate cmg based on pairwise loss function type
	 * @param losstype
	 * @param xuij
	 * @return
	 */
	protected double getGradMag(int losstype, double xuij) {
		...
		case 2:// logistic loss, BPR
			...
			break;
		case 3:// Frank loss
			cmg = Math.sqrt(g(xuij)) / (1 + Math.exp(xuij));
			break;
	...
	protected Map<Measure, Double> evalRankings() throws Exception {
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			SparseVector x_j = null;
			for (final Integer j : candItems) {
				// item j is not rated 
				if (!ratedItems.contains(j)) {
					final double rank;

					if (cf.getString("recommender").toLowerCase()
							.equals("rankfm_nextmusic"))
					{
						...
						String itemfield_j=rateDao.getItemId(j);//positive
	...
	protected double ranking(SparseVector x) throws Exception {
		return predict(x, false);
	}
	protected double predict(SparseVector x, boolean bound) throws Exception {
